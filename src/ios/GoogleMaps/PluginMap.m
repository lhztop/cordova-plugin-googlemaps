...
- (void)setCameraTarget:(CDVInvokedUrlCommand *)command {
  ...
  [self.mapCtrl.executeQueue addOperationWithBlock:^{
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
      [self.mapCtrl.map setCenter:CGPointMake(latitude, longitude)];
    }];
...
- (void)setMyLocationEnabled:(CDVInvokedUrlCommand *)command {
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    self.mapCtrl.map.showsUserLocation = [[params valueForKey:@"myLocation"] boolValue];
  }];
...
- (void)setTrafficEnabled:(CDVInvokedUrlCommand *)command {
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    Boolean isEnabled = [[command.arguments objectAtIndex:0] boolValue];
    self.mapCtrl.map.showTraffic = isEnabled;
  }];
...
- (void)setCameraTilt:(CDVInvokedUrlCommand *)command {
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    if (angle >=0 && angle <= 60) {
      [self.mapCtrl.map setCameraDegree:angle];
    }
...
- (void)setCameraBearing:(CDVInvokedUrlCommand *)command {
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    [self.mapCtrl.map setRotationDegree:bearing];
...
- (void)setAllGesturesEnabled:(CDVInvokedUrlCommand *)command {
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    Boolean isEnabled = [[command.arguments objectAtIndex:0] boolValue];
    [self.mapCtrl.map setZoomEnabled:isEnabled];
    [self.mapCtrl.map setScrollEnabled:isEnabled];
  }];
...
-(void)_changeCameraPosition: (NSString*)action requestMethod:(NSString *)requestMethod params:(NSDictionary *)json command:(CDVInvokedUrlCommand *)command {
  ...
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    if ([json valueForKey:@"bearing"] && [json valueForKey:@"bearing"] != [NSNull null]) {
      ...
      bearing = self.mapCtrl.map.rotationDegree;
    }
    ...
    if ([json valueForKey:@"tilt"] && [json valueForKey:@"tilt"] != [NSNull null]) {
      ...
      angle = self.mapCtrl.map.cameraDegree;
    }
    ...
    if ([json valueForKey:@"zoom"] && [json valueForKey:@"zoom"] != [NSNull null]) {
      ...
      zoom = self.mapCtrl.map.zoomLevel;
    }
...
//          } else {
//            if (bearing == 0) {
//              MACoordinateBounds *bounds = [[MACoordinateBounds alloc] initWithRegion:self.mapCtrl.map.projection.visibleRegion];
...
//      [self.mapCtrl.map setBounds:cameraBounds];
...
- (void)fromLatLngToPoint:(CDVInvokedUrlCommand*)command {
  ...
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    float longitude = [[command.arguments objectAtIndex:1] floatValue];
      ...
      [self.mapCtrl.view setCenter:point];
...
- (void)fromPointToLatLng:(CDVInvokedUrlCommand*)command {
  ...
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    [self.mapCtrl.executeQueue addOperationWithBlock:^{
      ...
      [latLngJSON addObject:[NSNumber numberWithDouble:pointX]];
      [latLngJSON addObject:[NSNumber numberWithDouble:pointY]];
  ...
  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    if (controls) {
      ...
      if ([controls valueForKey:@"compass"] != nil) {
        ...
        if (isEnabled == true) {
          self.mapCtrl.map.showsCompass = YES;
        } else {
      ...
      if ([controls valueForKey:@"myLocationButton"] != nil) {
        ...
        if (isEnabled == true) {
          ...
          self.mapCtrl.map.showsUserLocation = NO;
        }
      }
      //myLocation
...
//        [self.mapCtrl.map setPadding:newPadding];
      ...
      if ([preferences valueForKey:@"zoom"] != nil) {
        ...
        float minZoom = self.mapCtrl.map.minZoomLevel;
        float maxZoom = self.mapCtrl.map.maxZoomLevel;
        ...
        [self.mapCtrl.map setMinZoomLevel:minZoom];
        [self.mapCtrl.map setMaxZoomLevel:maxZoom];
      }
...
//      }
    ...
    if (styles) {
//      NSError *error;
//      Mapstyle *mapStyle = [GMSMapStyle styleWithJSONString:styles error:&error];
//      if (mapStyle != nil) {
...
//      }
      ...
      if (typeStr) {
        ...
        NSDictionary *mapTypes = [NSDictionary dictionaryWithObjectsAndKeys:
                                  ^() {return MAMapTypeBus; }, @"MAP_TYPE_HYBRID",
                                  ^() {return MAMapTypeSatellite; }, @"MAP_TYPE_SATELLITE",
...
//          // Change the map type
//          mapType = caseBlock();
          self.mapCtrl.map.mapType = mapTypes[typeStr];
//        }
      }
    }
